...
namespace Json
{
  namespace Rpc
  {
     ...
     * \brief JSON-RPC Http client.
     */
    class RestfulHttpClient : public Client
    {
      public:
        /**
         ...
         * will override.
         */
		RestfulHttpClient(const std::string& address);
         ...
         */
		RestfulHttpClient(const std::string& address, uint16_t port, const std::string& username, const std::string& password);
		...
		/*tang*/
		void ChangeUserpwd(const std::string& username, const std::string& password);
      ...
      private:   
         ...
         * \param userdata pointer to user data (in our case it is out class instance)
         * \return the size of the data received
         */
        static size_t StaticRecv(void *buffer, size_t size, size_t nmemb, void* userdata);
