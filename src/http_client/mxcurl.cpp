...
Session::Session() {
    ...
    if (curl) {
		...
		curl_easy_setopt(curl, CURLOPT_POST, 1);//tang
		...
		size_t error = curl_easy_setopt(curl, CURLOPT_HTTPHEADER, m_headers);
		if (error != 0)
		{
			//std::cerr << curl_easy_strerror((CURLcode)error);
			printf(curl_easy_strerror((CURLcode)error));
		}
...
//	payload=urlEncode(payload);
    ...
    if (curl) 
	{
        ...
        //curl_easy_setopt(curl, CURLOPT_COPYPOSTFIELDS, payload);//CURLOPT_POSTFIELDS
