...
namespace Json
{
  namespace Rpc
  {
    ...
    HttpClient::HttpClient(const std::string& address, uint16_t port, const std::string& username, const std::string& password)
    { 
      this->Initialize();
      this->ChangeAddress(address);
    ...
    void HttpClient::Initialize()
    {
      ...
      curl_easy_setopt(m_curlHandle, CURLOPT_WRITEFUNCTION, HttpClient::StaticRecv);
    ...
    HttpClient::~HttpClient()
    {
      ...
      curl_easy_cleanup(m_curlHandle);//by tang
      curl_global_cleanup();
    }
	...
	void HttpClient::ChangeUserpwd(const std::string& username, const std::string& password)
	{
    ...
    ssize_t HttpClient::Send(const std::string& data)
    {
      ...
      if((error = (size_t)curl_easy_setopt(m_curlHandle, CURLOPT_URL, this->GetAddress().c_str())) != 0)
      {
        std::cerr << curl_easy_strerror((CURLcode)error);
        return error;
      }
	...
	*/
    size_t HttpClient::StaticRecv(void *buffer, size_t size, size_t nmemb, void* userdata)
    {
      ...
      static_cast<HttpClient*>(userdata)->StoreRecv(data);
    ...
    ssize_t HttpClient::Recv(std::string &data)
    {
	  ...
	  //by tang  "id":1010} 为结束标志
	  std::string end_flag = "\"id\":1010}";
	  std::string end_str = data.substr(data.length() - end_flag.length());
	  while (end_str != end_flag && !m_lastReceivedList.empty())//判断json字符串是否结束
	  {
		  data += this->m_lastReceivedList.front();
